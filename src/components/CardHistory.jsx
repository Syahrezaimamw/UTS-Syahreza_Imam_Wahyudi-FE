...
const CardHistory = ({ peran, sub, e, cancel }) => {
    return (
        <div className="relative flex-col transition-shadow duration-300 bg-white border rounded-lg shadow-sm cursor-pointer w-[280px] w-flex group border-slate-200 hover:shadow-lg">
            ...
            <div className="p-4 py-0 ">
                {peran === 'admin' ?
                    <div className='flex justify-center w-full gap-2'>
                        <p className='text-lg font-semibold'>{e.User.nama}</p>
                        <p className='text-lg font-semibold'>({e.User.no_ktp})</p>
                    </div> : <></>}
            ...
            <div className={`w-full gap-2 px-3 pb-2 `} >
                ...
                ? "" :   <div onClick={peran === "admin" ? () => sub(e) : ''} className={`${e.status === "Diminta" ?
